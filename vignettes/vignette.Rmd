...
We will also need poststratification population counts and region names (\code{poststrat} and \code{code_region} datasets). Following actual study results, we exclude one region (Mayotte).
```{r extdata}
...
### MCMC
Next, posteriors of regression parameters are estimated by MCMC
```{r fit}
